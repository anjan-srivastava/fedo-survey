...
(function() {
   var clientWidth = window.document.documentElement.clientWidth,
       ...
       PLUGIN_WIDTH = IS_MOBILE? clientWidth: 340,
    ...
    init = function (document, window) {
        ...
        renderReview = function(model) {
            ...
            renderCard = function() {
                ...
                if (config.settings.titleEnabled) {
                    ...
                    titleBar.attr('style', 'border:none');
                }

                titleBar.append(onode('i.fa.close'));

                return card.append(titleBar)
        ...
        setupActions = function(widgetNode) {
            ...
            widgetNode.append(onode('script').attr('src', '#{staticUrl}/siema.min.js').attr('onload', function() {
                    ...
                    autoHeight = function() {
                        ...
                        for (i=0; i<slides.length; i+=1) {
                            if (this.currentSlide == i) {
                                var z = window.getComputedStyle(slides[i]).height;
                                slider.style.height = z;
                                window.setTimeout(function() { iframe.style.height = z;}, 200);
                                break;
                            }
                        }
                ...
                var siema = new window.Siema({
                    duration: 200,
                    onInit: function() { var that=this; window.setTimeout(function() { autoHeight.apply(that, null); mainCont.style.visibility = 'visible';}, 1000)},
                ...
                var isOpen, 
                    head = widgetNode.domNode.getElementsByClassName('head')[0],
                    ...
                    autoScroll = function() {
                        var handle = null,
                            ...
                            pause = function() { window.clearInterval(handle); handle = null; }
                        return function (enabled) {
                            if (!handle && enabled) play();
                            else pause();
                        };
                    }();
                ...
                if (config.settings.defaultState) {
                    ...
                    iframe.style.clip = 'rect(0px '+ PLUGIN_WIDTH +'px 380px 0px)';
                    if (config.settings.scrollEnabled) autoScroll(true);
                } else {
                    isOpen = false;
                    content.style.visibility = 'hidden';
                    content.style.opacity = 0;
                    setTimeout(function() { iframe.style.clip = calcClipClosed(); }, 1500);
                }
                
                content.addEventListener('mouseenter', function() { autoScroll(false); }, false);
                content.addEventListener('mouseleave', function() { isOpen && config.settings.scrollEnabled && autoScroll(true); }, false);

                Array.from(close).forEach(function (elem) {
                    elem.addEventListener('click', function(e) {
                        isOpen = false;
                        content.style.opacity = 0;
                        iframe.style.clip = calcClipClosed();
        ...
        try {
            ...
            mainCont.style.width = PLUGIN_WIDTH + 'px'; // this is important for slider to work correctly.
    ...
    iframe.style = 'border:none;position:fixed;overflow:hidden;z-index:9999';
    ...
    iframe.onload = function() {
        ...
        iframeDoc.open();
        iframeDoc.write("<html><body "+ cls +"style='overflow:hidden'><div id='outreech-widget-container' class='place-bottom-left'></div></body></html>");
        iframeDoc.close();
